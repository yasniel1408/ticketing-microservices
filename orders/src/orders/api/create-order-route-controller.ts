...
import {OrderCreatedPublisher} from "@app/orders/events/publishers/order-created-publisher";
    ...
    configureRoutes(): express.Application {
        this.app.post(
            ...
            async (req: Request, res: Response) => {
                ...
                //publicar el evento cuando es creada la orden
                await new OrderCreatedPublisher(NatsClientWrapper.client).publish({
                    id: order.id,
                    status: order.status,
                    ticket: {
                        id: order.ticket.id,
                        price: order.ticket.price.valueOf(),
                    },
